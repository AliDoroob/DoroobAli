...
<div class="container" style="margin-top: 20px;">
    <div class="row">
        @foreach ($news as $newsItem) <!-- Replace $news with your news data variable -->
        <div class="col-md-3 mb-4">
            <div class="custom-card">
                <img src="{{ asset('images/' . $newsItem->image) }}" class="card-img-top" alt="{{ $newsItem->title }}">
                <div class="card-body">
                    <h6 class="card-title text-right">
                  <a href="{{ route('newsown', ['id' => $newsItem->id]) }}" style="text-decoration: none; color: #333;">{{ $newsItem->name }}</a>
                    </h6>
                    <p class="card-text text-right">{{ $newsItem->description }}</p>
                </div>
            </div>
        </div>
        @endforeach
